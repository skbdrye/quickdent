    ...
    <>
          ...
          <div className="absolute right-0 top-full mt-2 w-80 sm:w-96 bg-card border border-border rounded-xl shadow-lg z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
            <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-muted/30">
              ...
              <div className="flex items-center gap-1">
                {unreadCount > 0 && (
                  <Button variant="ghost" size="sm" className="h-7 text-xs gap-1 text-muted-foreground hover:text-foreground" onClick={() => setConfirmMarkAll(true)}>
                    <CheckCheck className="w-3.5 h-3.5" /> Read all
                  </Button>
                )}
                {notifications.length > 0 && (
                  <Button variant="ghost" size="sm" className="h-7 text-xs gap-1 text-muted-foreground hover:text-destructive" onClick={() => setConfirmClear(true)}>
                  ...
                  {notifications.map((n: Notification) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-muted-foreground/60 mt-1">{formatNotificationTime(n.created_at)}</p>
                      </div>
                      {!n.is_read && (
      ...
      <AlertDialog open={confirmClear} onOpenChange={setConfirmClear}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => { if (user?.id) { clearAll(user.id); setOpen(false); } setConfirmClear(false); }}>
              Clear All
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      <AlertDialog open={confirmMarkAll} onOpenChange={setConfirmMarkAll}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => { if (user?.id) markAllAsRead(user.id); setConfirmMarkAll(false); }}>
              Mark All Read
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
